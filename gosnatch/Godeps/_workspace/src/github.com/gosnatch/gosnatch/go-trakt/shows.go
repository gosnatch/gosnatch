...
var (
    ...
    ShowsRemoteIdURL = Hyperlink("search?id_type={type}&id={remoteId}")
...
func (r *ShowsService) Tvdb(tvdbID int) (show []ShowResult, result *Result) {
    url, _ := ShowsRemoteIdURL.Expand(M{"type": "tvdb", "remoteId": fmt.Sprintf("%d", tvdbID)})
    result = r.client.get(url, &show)
    return
}
